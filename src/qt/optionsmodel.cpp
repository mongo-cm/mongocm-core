...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MONGOcm);
    ...
    if (!settings.contains("nAnonymizeMONGOCMAmount"))
        settings.setValue("nAnonymizeMONGOCMAmount", 1000);

    nAnonymizeMONGOCMAmount = settings.value("nAnonymizeMONGOCMAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMONGOCMAmount"))
        SoftSetArg("-anonymizemongocmamount", settings.value("nAnonymizeMONGOCMAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mongocm.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMONGOCMAmount:
            return QVariant(nAnonymizeMONGOCMAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMONGOCMAmount:
            nAnonymizeMONGOCMAmount = value.toInt();
            settings.setValue("nAnonymizeMONGOCMAmount", nAnonymizeMONGOCMAmount);
            emit anonymizeMONGOCMAmountChanged(nAnonymizeMONGOCMAmount);
            break;
        case CoinControlFeatures:
