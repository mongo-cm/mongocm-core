...
#include "zmongocmcontroldialog.h"
#include "ui_zmongocmcontroldialog.h"
...
std::list<std::string> ZMONGOCMControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZMONGOCMControlDialog::listMints;

ZMONGOCMControlDialog::ZMONGOCMControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMONGOCMControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZMONGOCMControlDialog::~ZMONGOCMControlDialog()
{
    delete ui;
}

void ZMONGOCMControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMONGOCMControlDialog::updateList()
{
...
void ZMONGOCMControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMONGOCMControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZMONGOCM_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));
}

std::vector<CZerocoinMint> ZMONGOCMControlDialog::GetSelectedMints()
{
...
void ZMONGOCMControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
