...
#if defined(NDEBUG)
#error "MONGOcm cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zMONGOCM spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zMONGOCM spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zMONGOCM spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight > 200 && nHeight <= 20160) { // phase 1
        nSubsidy = 275 * COIN;
    }
	else if (nHeight > 20160 && nHeight <= 50400) { // phase 2
        nSubsidy = 290 * COIN;
    }
	else if (nHeight > 50400 && nHeight <= 100800) { // phase 3
        nSubsidy = 305 * COIN;
    }
	else if (nHeight > 100800 && nHeight <= 151200) { // phase 4
        nSubsidy = 330 * COIN;
    }
	else if (nHeight > 151200 && nHeight <= 201600) { // phase 5
        nSubsidy = 370 * COIN;
    }
	else if (nHeight > 201600 && nHeight <= 252000) { // phase 6
        nSubsidy = 415 * COIN;
    }
	else if (nHeight > 252000 && nHeight <= 277200) { // phase 7
        nSubsidy = 455 * COIN;
    }
	else if (nHeight > 277200 && nHeight <= 302400) { // phase 8
        nSubsidy = 495 * COIN;
    }
	else if (nHeight > 302400 && nHeight <= 327600) { // phase 9
        nSubsidy = 515 * COIN;
    }
	else if (nHeight > 327600 && nHeight <= 352800) { // phase 10
        nSubsidy = 540 * COIN;
    }
	else if (nHeight > 352800 && nHeight <= 378000) { // phase 11
        nSubsidy = 555 * COIN;
    }
	else if (nHeight > 378000 && nHeight <= 403200) { // phase 12
        nSubsidy = 580 * COIN;
    }
	else if (nHeight > 403200 && nHeight <= 428400) { // phase 13
        nSubsidy = 605 * COIN;
    }
	else if (nHeight > 428400 && nHeight <= 453600) { // phase 14
        nSubsidy = 620 * COIN;
    }
	else if (nHeight > 453600 && nHeight <= 478800) { // phase 15
        nSubsidy = 640 * COIN;
    }
	else if (nHeight > 478800 && nHeight <= 504000) { // phase 16
        nSubsidy = 655 * COIN;
    }
	else if (nHeight > 504000 && nHeight <= 554400) { // phase 17
        nSubsidy = 680 * COIN;
    }
	else if (nHeight > 554400 && nHeight <= 604800) { // phase 18
        nSubsidy = 705 * COIN;
    }
	else if (nHeight > 604800 && nHeight <= 655200) { // phase 19
        nSubsidy = 720 * COIN;
    }
	else if (nHeight > 655200 && nHeight <= 705600) { // phase 20
        nSubsidy = 745 * COIN;
    }
	else if (nHeight > 705600 && nHeight <= 756000) { // phase 21
        nSubsidy = 790 * COIN;
    }
	else if (nHeight > 756000 && nHeight <= 806400) { // phase 22
        nSubsidy = 745 * COIN;
    }
	else if (nHeight > 806400 && nHeight <= 856800) { // phase 23
        nSubsidy = 665 * COIN;
    }
	else if (nHeight > 856800 && nHeight <= 907200) { // phase 24
        nSubsidy = 580 * COIN;
    }
	else if (nHeight > 907200 && nHeight <= 957600) { // phase 25
        nSubsidy = 495 * COIN;
    }
	else if (nHeight > 957600 && nHeight <= 1008000) { // phase 26
        nSubsidy = 415 * COIN;
    }
	else if (nHeight > 1008000 && nHeight <= 1134000) { // phase 27
        nSubsidy = 370 * COIN;
    }
	else if (nHeight > 1134000 && nHeight <= 1260000) { // phase 28
        nSubsidy = 330 * COIN;
    }
	else if (nHeight > 1260000 && nHeight <= 1386000) { // phase 29
        nSubsidy = 305 * COIN;
    }	
	else if (nHeight > 1386000 && nHeight <= 1512000) { // phase 30
        nSubsidy = 290 * COIN;
    }
	else if (nHeight > 1512000 && nHeight <= 21383340) { // phase 31
        nSubsidy = 165 * COIN;
    }	
	else if (nHeight > 21383340) {
        nSubsidy = 0.00 * COIN;
    }
    else {
        nSubsidy = 165 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from MONGOcm
...
{
    RenameThread("mongocm-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZMONGOCMMinted()
{
...
void RecalculateZMONGOCMSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zMONGOCM supply
...
bool RecalculateMONGOCMSupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zMONGOCM with serial %s is already in the block %d\n",
    ...
    if (!fVerifyingBlocks && pindex->nHeight == Params().Zerocoin_StartHeight() + 1) {
        RecalculateZMONGOCMMinted();
        RecalculateZMONGOCMSpent();
        RecalculateMONGOCMSupply(1);
    }
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zMONGOCMSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MONGOcm to zMONGOCM
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zMONGOCM spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zMONGOCM serial %s in block\n Block: %s",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if(IsSporkActive(SPORK_20_SPAM_CHK) && (block.GetBlockTime() >= GetSporkValue(SPORK_20_SPAM_CHK))) {
      ...
      if (block.IsProofOfStake()) {
          ...
          // Inputs
          std::vector<CTxIn> mongocmInputs;
          std::vector<CTxIn> zMONGOCMInputs;

          for (CTxIn stakeIn : stakeTxIn.vin) {
              if(stakeIn.scriptSig.IsZerocoinSpend()){
                  zMONGOCMInputs.push_back(stakeIn);
              }else{
                  mongocmInputs.push_back(stakeIn);
              }
          }
          const bool hasMONGOCMInputs = !mongocmInputs.empty();
          const bool hasZMONGOCMInputs = !zMONGOCMInputs.empty();
          ...
          for (CTransaction tx : block.vtx) {
              for (CTxIn in: tx.vin) {
                  ...
                  if(hasMONGOCMInputs)
                      ...
                      for (CTxIn mongocmIn : mongocmInputs){
                          if(mongocmIn.prevout == in.prevout){
          ...
          if (isBlockFromFork) {
                ...
                // Check if the forked chain is longer than the max reorg limit
                  ...
                  for (CTransaction t : bl.vtx) {
                      for (CTxIn in: t.vin) {
                          ...
                          for (CTxIn stakeIn : mongocmInputs) {
              ...
              // Now that this loop if completed. Check if we have zMONGOCM inputs.
              if(hasZMONGOCMInputs){
                  for (CTxIn zPawsInput : zMONGOCMInputs) {
                      ...
                      //                                stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zmongocm");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zMONGOCM mints and %d zMONGOCM spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // MONGOcm: We use certain sporks during IBD, so check to see if they are
...
{
	return CScript() << ParseHex("0485128f0d02bf55f12601d69bbfbc27f958e8f033a3be19fe90d6ee35cedfbd1d531ad66c391f7eefd23c49b12de2b596661214ca46e38a12740d4f17a7bae5c2") << OP_CHECKSIG;
}
