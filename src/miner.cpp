...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zMONGOCM spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zMONGOCM serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("MONGOCMMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("MONGOCMMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("MONGOCMMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mongocm-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running MONGOCMMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("MONGOCMMiner() exception");
    } catch (...) {
        LogPrintf("MONGOCMMiner() exception");
    }

    LogPrintf("MONGOCMMiner exiting\n");
}
